    ...
    namespace DebugDraw
    {
        ...
        void OpenJoltSettingsWindow()
        {
            ...
            AzToolsFramework::EditorRequestBus::Broadcast(&AzToolsFramework::EditorRequests::OpenViewPane,
                LyViewPane::JoltConfigurationEditor);
        ...
        bool IsGlobalColliderDebugCheck(Debug::DebugDisplayData::GlobalCollisionDebugState requiredState)
        {
            ...
            {
                return joltDebug->GetDebugDisplayData().m_globalCollisionDebugDraw == requiredState;
            }
            return false;
        }
            ...
            if (auto serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
            {
                ...
                if (auto editContext = serializeContext->GetEditContext())
                {
                    ...
                    editContext->Class<Collider>(
                        "Jolt Collider Debug Draw", "Global and per-collider debug draw preferences.")
                        ...
                        ->UIElement(AZ::Edit::UIHandlers::Button, "Draw collider",
                            ...
                            ->Attribute(AZ::Edit::Attributes::ButtonTooltip,
                                ...
                                "set the Global Collision Debug setting to \"Set manually\" in the Jolt Configuration)."
                                "'Draw Helpers' needs to be enabled to use.")
                            ->Attribute(AZ::Edit::Attributes::Visibility,
                                VisibilityFunc{ []() { return !IsGlobalColliderDebugCheck(GlobalCollisionDebugState::Manual); } })
                            ->Attribute(AZ::Edit::Attributes::ChangeNotify, &OpenJoltSettingsWindow)
                            ->Attribute(AZ::Edit::Attributes::ReadOnly, &IsDrawColliderReadOnly)
                        ;
                }
            }
        ...
        Collider::Collider()
            : m_debugDisplayDataChangedEvent(
                  [this]([[maybe_unused]] const JoltPhysics::Debug::DebugDisplayData& data)
                  {
        ...
        void Collider::BuildMeshes(const Physics::ShapeConfiguration& shapeConfig, AZ::u32 geomIndex) const
        {
            ...
            {
                AZ_Warning("Jolt Debug Draw", false, "Mesh has too many triangles! Entity: '%s', ID: %llu",
            ...
            , const ElementDebugInfo& elementDebugInfo) const
        {
            ...
            if (auto* joltDebug = AZ::Interface<Debug::JoltDebugInterface>::Get())
            {
                debugDrawColorMode = joltDebug->GetDebugDisplayData().m_globalCollisionDebugDrawColorMode;
            }
            ...
            AZ::u32 geomIndex) const
        {
            ...
            {
                AZ_Error("Jolt", false, "DrawMesh: geomIndex %d is out of range for %s. Size: %d",
            ...
            AzFramework::DebugDisplayRequests& debugDisplay)
        {
            ...
            auto* joltDebug = AZ::Interface<Debug::JoltDebugInterface>::Get();
            if (joltDebug == nullptr)
            {
                return;
            }
            const JoltPhysics::Debug::DebugDisplayData& displayData = joltDebug->GetDebugDisplayData();

            const JoltPhysics::Debug::ColliderProximityVisualization& proximityVisualization = displayData.m_colliderProximityVisualization;
            ...
            const JoltPhysics::Debug::DebugDisplayData::GlobalCollisionDebugState& globalCollisionDebugDraw = displayData.m_globalCollisionDebugDraw;
            if (globalCollisionDebugDraw != JoltPhysics::Debug::DebugDisplayData::GlobalCollisionDebugState::AlwaysOff)
            {
                if (globalCollisionDebugDraw == JoltPhysics::Debug::DebugDisplayData::GlobalCollisionDebugState::AlwaysOn
                    || m_locallyEnabled
        ...
        void Collider::OnSelected()
        {
            ...
            if (auto* joltDebug = AZ::Interface<Debug::JoltDebugInterface>::Get())
            {
                joltDebug->RegisterDebugDisplayDataChangedEvent(m_debugDisplayDataChangedEvent);
            }
        }

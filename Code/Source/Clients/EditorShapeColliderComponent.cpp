...
namespace JoltPhysics
{
    ...
    void EditorShapeColliderComponent::UpdateCachedSamplePoints() const
    {
        ...
        default:
            AZ_WarningOnce("Jolt Shape Collider Component", false, "Unsupported shape type in UpdateCachedSamplePoints");
            break;
        }
    ...
    void EditorShapeColliderComponent::UpdateShapeConfigs()
    {
        ...
        else
        {
            ...
            AZ_Warning("Jolt Shape Collider Component", m_shapeTypeWarningIssued, "Unsupported shape type for "
    ...
    void EditorShapeColliderComponent::UpdateCylinderConfig(const float scale)
    {
        ...
        if (radius <= 0.0f || height <= 0.0f)
        {
            ...
            AZ_Warning("Jolt", false, "%s: Negative or zero cylinder dimensions are invalid (radius: '%f', height: '%f).", GetEntity()->GetName().c_str(), radius, height);
            return;
        }
        ...
        if (!points.has_value())
        {
            ...
            AZ_Warning("Jolt", false, "Could not generate cylinder shape collider.");
            return;
        }
    ...
    void EditorShapeColliderComponent::Activate()
    {
        ...
        JoltPhysics::ColliderShapeRequestBus::Handler::BusConnect(GetEntityId());
    ...
    void EditorShapeColliderComponent::OnSelected()
    {
        ...
        {
            if (!m_joltConfigChangedHandler.IsConnected())
            {

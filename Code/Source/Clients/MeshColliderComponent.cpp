...
namespace JoltPhysics
{
    ...
    void MeshColliderComponent::Activate()
    {
        ...
        AZ_Error("Jolt", validConfiguration, "Expected exactly one collider/shape configuration for entity \"%s\".",
            GetEntity()->GetName().c_str());

        if (validConfiguration)
        {
            validConfiguration = m_shapeConfigList[0].second->GetShapeType() == Physics::ShapeType::PhysicsAsset;
            AZ_Error("Jolt", validConfiguration, "Expected shape configuration to be of type PhysicsAsset for entity \"%s\".",
                GetEntity()->GetName().c_str());
        }
    ...
    void MeshColliderComponent::UpdateScaleForShapeConfigs()
    {
        ...
        {
            AZ_Error("Jolt Mesh Collider Component", false,

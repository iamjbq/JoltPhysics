...
namespace JoltPhysics
{
    namespace Pipeline
    {
        const char* MeshAssetHandler::s_assetFileExtension = "joltmesh";
        ...
        {
            return "Jolt Collision Mesh (JoltPhysics Gem)";
        }
            ...
            const AZ::Data::AssetFilterCB& assetLoadFilterCB)
        {
            ...
            if (!meshAsset)
            {
                AZ_Error("Jolt Mesh Asset", false, "This should be a JoltMeshAsset, as this is the only type that can be processed.");
            ...
            // Jolt Mesh Asset could have been saved with material assets that only have the hint valid (asset path in the cache).
            // This could happen for example when MeshGroup was filled procedurally and the materials were only given the path (hint).
            // Now at runtime after the Jolt Mesh is loaded let's complete the asset by looking for it in the catalog and
        ...
        void MeshAsset::Reflect(AZ::ReflectContext* context)
        {
            ...
            if (auto* serializeContext = azrtti_cast<AZ::SerializeContext*>(context))
            {
                ...
                {
                    editContext->Class<MeshAsset>(QT_TRANSLATE_NOOP("JoltPhysics", "Jolt Mesh Asset"), "")

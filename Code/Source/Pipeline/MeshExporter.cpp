...
// #include <Pipeline/PrimitiveShapeFitter/PrimitiveShapeFitter.h>
...
namespace JoltPhysics
{
    namespace Pipeline
    {
        ...
        void MeshExporter::Reflect(AZ::ReflectContext* context)
        {
            ...
            if (serializeContext)
            {
                serializeContext->Class<MeshExporter, AZ::SceneAPI::SceneCore::ExportingComponent>()
                    ->Version(1 + (1<<JPH_VERSION_MINOR)); // Use Jolt minor version to trigger assets recompilation
            }
        }
        ...
        static AZ::SceneAPI::Events::ProcessingResult WriteJoltMeshAsset(
            ...
            const MeshGroup& meshGroup)
        {
            ...
            for (const NodeCollisionGeomExportData& subMesh : totalExportData)
            {
                ...
                else
                {
                    AZ_Assert(
                        ...
                        "WriteJoltMeshAsset: m_perFaceMaterialIndices must be not empty! Please make sure you have a material assigned to the geometry. Node: %s",
                        subMesh.m_nodeName.c_str()
                    );
                    AZ_Assert(
                        subMesh.m_perFaceMaterialIndices[0] != MeshAssetData::TriangleMeshMaterialIndex,
                        "WriteJoltMeshAsset: m_perFaceMaterialIndices has invalid material index! Node: %s",
                        subMesh.m_nodeName.c_str()
                    );
            ...
            if (JoltPhysics::Utils::WriteCookedMeshToFile(filename, assetData))
            {
        ...
        SceneEvents::ProcessingResult MeshExporter::ProcessContext(SceneEvents::ExportEventContext& context) const
        {
            ...
            for (const MeshGroup& joltMeshGroup : view)
            {
                ...
                {
                    const ConvexDecompositionParams& convexDecompositionParams = joltMeshGroup.GetConvexDecompositionParams();
                ...
                sceneNodeSelectionList.EnumerateSelectedNodes(
                    [&](const AZStd::string& name)
                    {
                        ...
                        const AZ::SceneAPI::DataTypes::MatrixType worldTransform = coordSysConverter.ConvertMatrix3x4(
                            AZ::SceneAPI::Utilities::DetermineWorldTransform(scene, nodeIndex, joltMeshGroup.GetRuleContainerConst()));
                ...
                {
                    result += WriteJoltMeshAsset(context, totalExportData, *assetMaterialData, joltMeshGroup);
                }
            }
